(define (problem test-template)
	...
	(:init
		...
		(FirstAssigned Init) 
		(SecondAssigned Init) 
		(FirstType InitF InitT) 
		(SecType InitS InitT)
		(DayFirst Init InitF) 
		(DaySecond Init InitS) 
		(PrevDay Init Monday) (FirstUsed InitF)
		(SecUsed InitS)
		
		; Days order
		(PrevDay Monday Tuesday) 
		(PrevDay Tuesday Wednesday) 
		(PrevDay Wednesday Thursday) 
		(PrevDay Thursday Friday)
